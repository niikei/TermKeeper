...
from termkeeper.application.errors import InitializationError, ValidationError
...
class SystemUseCases:
    ...
    def diagnostics(self) -> SystemDiagnostics:
        try:
            ...
            message = (
                "Could not inspect the TermKeeper database. "
                "Run 'tk --debug doctor' for technical details."
            )
...
def _diagnostics() -> SystemDiagnostics:
    engine = get_engine()
